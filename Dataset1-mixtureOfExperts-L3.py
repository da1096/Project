...
### Architecture
architectureName = "MixtureOfExpertsCNN"
### Number of layers
layersNum = 3
### Running info
...
## Mixture of Experts CNN
class MixtureOfExpertsCNN(nn.Module):
    def __init__(self, num_experts=3):
        super(MixtureOfExpertsCNN, self).__init__()
        ...
        # Species
        self.species_experts = nn.ModuleList([Expert(128) for _ in range(num_experts)])
        self.species_gate = nn.Sequential(
            nn.Flatten(),
        ...
        # Health
        self.health_experts = nn.ModuleList([Expert(128) for _ in range(num_experts)])
        self.health_gate = nn.Sequential(
            nn.Flatten(),
            nn.Linear(128 * 28 * 28, 512),
            nn.ReLU(),
            nn.Linear(512, num_experts),
    ...
    def forward(self, x):
        ...
        # Species Mixture of Experts
        species_expert_outputs = [expert(features) for expert in self.species_experts] 
        species_gate_weights = self.species_gate(features)  
        species_gate_weights = species_gate_weights.unsqueeze(1).unsqueeze(-1).unsqueeze(-1)  
        species_expert_outputs = torch.stack(species_expert_outputs, dim=2) 
        species_features = (species_expert_outputs * species_gate_weights).sum(dim=2)  
        
        # Health Mixture of Experts
        health_expert_outputs = [expert(features) for expert in self.health_experts]  
        health_gate_weights = self.health_gate(features)  
        health_gate_weights = health_gate_weights.unsqueeze(1).unsqueeze(-1).unsqueeze(-1)  
        health_expert_outputs = torch.stack(health_expert_outputs, dim=2)  
        health_features = (health_expert_outputs * health_gate_weights).sum(dim=2)  
        
        # Predictions
        species_pred = self.species_head(species_features)
        health_pred = self.health_head(health_features)
...
def main():
    ...
    model = MixtureOfExpertsCNN(num_experts=3)
    ...
    # Save Model
    torch.save(model.state_dict(), 'Dataset1-mixtureOfExperts-L3.pth')
