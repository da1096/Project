...
### Dataset
datasetName = "Dataset-3/Augmented_Dataset"
### Architecture
architectureName = "SharedBackboneCNN"
### Number of layers
layersNum = 7
### Running info
...
class MultiTaskDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        # Define mappings for species and health
        ...
        # Sort species by length (longest first) to prioritize full matches
        ...
        for folder_name in os.listdir(root_dir):
            ...
            if os.path.isdir(folder_path):
                # Find species by checking full prefix match
                species = None
                for s in self.species_list:
                    if folder_name.startswith(s):
                        # Ensure the next character is an underscore or end of string
                ...
                # Extract health as the part after the species
...
class SharedBackboneCNN(nn.Module):
    def __init__(self):
        super(SharedBackboneCNN, self).__init__()
        self.backbone = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(256),
            nn.MaxPool2d(2, 2),                            # 28x28x256 -> 14x14x256
            nn.Conv2d(256, 512, kernel_size=3, padding=1), # 14x14x256 -> 14x14x512
            nn.ReLU(),
            nn.BatchNorm2d(512),
            nn.MaxPool2d(2, 2),                            # 14x14x512 -> 7x7x512
            nn.Conv2d(512, 512, kernel_size=3, padding=1), # 7x7x512 -> 7x7x512
            nn.ReLU(),
            nn.BatchNorm2d(512),
            nn.MaxPool2d(2, 2),                            # 7x7x512 -> 3x3x512
            nn.Conv2d(512, 512, kernel_size=3, padding=1), # 3x3x512 -> 3x3x512
            nn.ReLU(),
            nn.BatchNorm2d(512),
            nn.MaxPool2d(3, 3)                             # 3x3x512 -> 1x1x512
        )
        
        self.species_head = nn.Sequential(
            nn.Flatten(),
            nn.Linear(512 * 1 * 1, 512),                  # Adjusted for 1x1x512
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(512, 5)
        )
    ...
    def forward(self, x):
        ...
        health_pred = self.health_head(features)
...
def main():
    ...
    torch.save(model.state_dict(), 'Dataset3-sharedbackbone-L3.pth')
