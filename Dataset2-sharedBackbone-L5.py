...
def train_model(model, train_loader, val_loader, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        print(f'Training Loss: {train_loss:.4f}, Species Loss: {train_species_loss:.4f}, Health Loss: {train_health_loss:.4f}')
        print(f'Training Species Acc: {train_species_acc:.2f}%, Health Acc: {train_health_acc:.2f}%')
        print(f'Validation Loss: {val_loss:.4f}, Species Loss: {val_species_loss:.4f}, Health Loss: {val_health_loss:.4f}')
...
def main():
    ...
    # Combined metrics
    combined_preds = (all_species_preds * 5 + all_health_preds)
    combined_labels = (all_species_labels * 5 + all_health_labels)
    combined_metrics = compute_metrics(combined_preds, combined_labels, num_classes=20, task_name="Combined")
    ...
    # Results
    print('Test Results:')
    print(f'Test Loss: {test_loss:.4f}, Species Loss: {test_species_loss:.4f}, Health Loss: {test_health_loss:.4f}')
    ...
    # Save Model
    torch.save(model.state_dict(), 'Dataset2-sharedBackbone-L5.pth')
