...
### Architecture
architectureName = "CrossStitchCNN"
### Number of layers
layersNum = 7
### Running info
...
class MultiTaskDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        species_map = {"Cherry": 0, "Grape": 1, "Pepper_bell": 2, "Strawberry": 3, "Tomato": 4}
        health_map = {
...
## Cross Stitch CNN
class CrossStitchCNN(nn.Module):
    def __init__(self):
        super(CrossStitchCNN, self).__init__()
        # Species
        self.species_conv1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1),    
            nn.ReLU(),
            nn.BatchNorm2d(32),
            nn.MaxPool2d(2, 2)                             
        )
        self.species_conv2 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=3, padding=1),  
            nn.ReLU(),
            nn.BatchNorm2d(64),
            nn.MaxPool2d(2, 2)                             
        )
        self.species_conv3 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=3, padding=1),  
            nn.ReLU(),
            nn.BatchNorm2d(128),
            nn.MaxPool2d(2, 2)                             
        )
        self.species_conv4 = nn.Sequential(
            nn.Conv2d(128, 256, kernel_size=3, padding=1), 
            nn.ReLU(),
            nn.BatchNorm2d(256),
            nn.MaxPool2d(2, 2)                             
        )
        self.species_conv5 = nn.Sequential(
            nn.Conv2d(256, 512, kernel_size=3, padding=1), 
            nn.ReLU(),
            nn.BatchNorm2d(512),
            nn.MaxPool2d(2, 2)                             
        )
        self.species_conv6 = nn.Sequential(
            nn.Conv2d(512, 512, kernel_size=3, padding=1), 
            nn.ReLU(),
            nn.BatchNorm2d(512),
            nn.MaxPool2d(2, 2)                             
        )
        self.species_conv7 = nn.Sequential(
            nn.Conv2d(512, 512, kernel_size=3, padding=1), 
            nn.ReLU(),
            nn.BatchNorm2d(512),
            nn.MaxPool2d(3, 3)                             
        ...
        # Health
        self.health_conv1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1),    
            nn.ReLU(),
            nn.BatchNorm2d(32),
            nn.MaxPool2d(2, 2)                             
        )
        self.health_conv2 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=3, padding=1),   
            nn.ReLU(),
            nn.BatchNorm2d(64),
            nn.MaxPool2d(2, 2)                             
        )
        self.health_conv3 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=3, padding=1), 
            nn.ReLU(),
            nn.BatchNorm2d(128),
            nn.MaxPool2d(2, 2)                             
        )
        self.health_conv4 = nn.Sequential(
            nn.Conv2d(128, 256, kernel_size=3, padding=1), 
            nn.ReLU(),
            nn.BatchNorm2d(256),
            nn.MaxPool2d(2, 2)                             
        )
        self.health_conv5 = nn.Sequential(
            nn.Conv2d(256, 512, kernel_size=3, padding=1), 
            nn.ReLU(),
            nn.BatchNorm2d(512),
            nn.MaxPool2d(2, 2)                             
        )
        self.health_conv6 = nn.Sequential(
            nn.Conv2d(512, 512, kernel_size=3, padding=1), 
            nn.ReLU(),
            nn.BatchNorm2d(512),
            nn.MaxPool2d(2, 2)                             
        )
        self.health_conv7 = nn.Sequential(
            nn.Conv2d(512, 512, kernel_size=3, padding=1), 
            nn.ReLU(),
            nn.BatchNorm2d(512),
            nn.MaxPool2d(3, 3)                             
        ...
        self.cs1 = nn.Parameter(torch.ones(32, 2, 2) * 0.5)  
        self.cs2 = nn.Parameter(torch.ones(64, 2, 2) * 0.5)  
        self.cs3 = nn.Parameter(torch.ones(128, 2, 2) * 0.5) 
        self.cs4 = nn.Parameter(torch.ones(256, 2, 2) * 0.5) 
        self.cs5 = nn.Parameter(torch.ones(512, 2, 2) * 0.5) 
        self.cs6 = nn.Parameter(torch.ones(512, 2, 2) * 0.5) 
        self.cs7 = nn.Parameter(torch.ones(512, 2, 2) * 0.5) 
    ...
    def cross_stitch(self, x_s, x_h, cs_weights):
        batch_size, channels, height, width = x_s.size()
        x_s_flat = x_s.view(batch_size, channels, -1)
        x_h_flat = x_h.view(batch_size, channels, -1)
        x_combined = torch.stack([x_s_flat, x_h_flat], dim=2)
        x_stitched = torch.einsum('bcij,ckl->bclj', x_combined, cs_weights)
        x_s_new = x_stitched[:, :, 0, :].view(batch_size, channels, height, width)
        x_h_new = x_stitched[:, :, 1, :].view(batch_size, channels, height, width)
        return x_s_new, x_h_new
    
    def forward(self, x):
        s1 = self.species_conv1(x)
        h1 = self.health_conv1(x)
        s1, h1 = self.cross_stitch(s1, h1, self.cs1)
        
        s2 = self.species_conv2(s1)
        h2 = self.health_conv2(h1)
        s2, h2 = self.cross_stitch(s2, h2, self.cs2)
        
        s3 = self.species_conv3(s2)
        h3 = self.health_conv3(h2)
        s3, h3 = self.cross_stitch(s3, h3, self.cs3)
        
        s4 = self.species_conv4(s3)
        h4 = self.health_conv4(h3)
        s4, h4 = self.cross_stitch(s4, h4, self.cs4)
        
        s5 = self.species_conv5(s4)
        h5 = self.health_conv5(h4)
        s5, h5 = self.cross_stitch(s5, h5, self.cs5)
        
        s6 = self.species_conv6(s5)
        h6 = self.health_conv6(h5)
        s6, h6 = self.cross_stitch(s6, h6, self.cs6)
        
        s7 = self.species_conv7(s6)
        h7 = self.health_conv7(h6)
        s7, h7 = self.cross_stitch(s7, h7, self.cs7)
        
        species_pred = self.species_head(s7)
        health_pred = self.health_head(h7)
...
def main():
    ...
    health_metrics = compute_metrics(all_health_preds, all_health_labels, num_classes=13, task_name="Health")
    combined_true = all_species_labels * 13 + all_health_labels
    combined_pred = all_species_preds * 13 + all_health_preds
    combined_metrics = compute_metrics(combined_pred, combined_true, num_classes=65, task_name="Combined Species_Health")
    ...
    # Save Model
    torch.save(model.state_dict(), 'Dataset1-crossStitch-L7.pth')
