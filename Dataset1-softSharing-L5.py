...
### Architecture
architectureName = "SoftSharingCNN"
### Number of layers
layersNum = 5
### Running info
...
## Soft Sahring CNN
class SoftSharingCNN(nn.Module):
    def __init__(self):
        super(SoftSharingCNN, self).__init__()
        # Species
        self.species_conv = nn.Sequential(
        ...
        # Health
        self.health_conv = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1),    
            nn.ReLU(),
            nn.BatchNorm2d(32),
            nn.MaxPool2d(2, 2),                            
    ...
    def forward(self, x):
        species_features = self.species_conv(x)
        health_features = self.health_conv(x)
...
def main():
    ...
    # Save Model
    torch.save(model.state_dict(), 'Dataset1-softSharing-L5.pth')
